...
namespace S7.Net
{
    ...
    public partial class Plc
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                // Log de trama enviada
                if (_enableFrameLogging)
                {
                    var dataToLog = new byte[length];
                    Array.Copy(requestData, offset, dataToLog, 0, length);
                    LogFrame(FrameDirection.Sent, dataToLog, "TSDU Request");
                }
                ...
                // Log de trama recibida
                if (_enableFrameLogging && response != null)
                {
                    LogFrame(FrameDirection.Received, response, "TSDU Response");
                }
